"""
(已升级) 此文件包含所有AI Agent的任务指令。
这些指令提供了丰富的上下文、角色扮演和创作原则，
与 `schemas.py` 中定义的严格数据结构协同工作。
"""

# ===================================================================
# == 创作总纲 (所有Agent都需遵守)
# ===================================================================
# 1. 现实原则: 虽然故事包含超自然元素，但所有情节设计、角色动机和行为逻辑必须
#    最大程度地尊重现实世界的物理和人性规律，这是保证读者沉浸感和“爽点”的基础。
# 2. 元叙事约束: 元叙事手法（如角色意识到自己被操控）必须作为隐藏伏笔被极其
#    谨慎和隐晦地使用，绝不能滥用，以避免透支创作空间和破坏故事张力。
# ===================================================================
...
选择时需考虑电影是否适合普通人生存，以及情节是否能提供足够的“爽点”。
"""
...
请为整个故事弧规划 10-15 个场景，确保叙事节奏张弛有度，并为主角提供清晰的成长弧光。
...
4.  **创意与适配:** “主角适配”的设定是否巧妙？“元叙事伏笔”的设置和触发时机是否足够隐晦和高明？
...
你是一位才华横溢、写作风格细腻客观的中文小说家。
你的任务是根据以下提供的**所有背景资料和剧本设定**，完成一个场景的故事章节。

**核心写作要求:**
- **演绎剧本:** 你的写作必须完全服务于剧本。所有情节都必须围绕**副标题**展开，并全力渲染预设的**核心情绪锚点**。
- **主角的主动性:** 主角江浩必须表现出强烈的求生意志，主动与NPC沟通、制定计划、尝试合作。
- **注入内心驱动力:** 通过简短的内心独白、疑问或权衡，来揭示角色**为什么**会做出某个决定。
...
FIRST_CHAPTER_ADDITIONAL_INSTRUCTIONS = "故事从主角在一个无聊的下午观看电影《{movie_name}》，然后意外穿越开始。请立即让他面临一个小冲突或谜题。 {meta_narrative_instruction}"
GENERAL_CHAPTER_ADDITIONAL_INSTRUCTIONS = "{meta_narrative_instruction}"
